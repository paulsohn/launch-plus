"""CLI for launch-plus — Bazel-like build and run system for ROS 2.

Ported from crates/launch-plus-cli/src/main.rs (clap → Click).
"""
...
if TYPE_CHECKING:
    from launch_plus.fetcher import WorkspaceState
...
def _read_colcon_flagfile(path: str) -> list[str]:
    ...
    for line in content.splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        # Reject flags managed by launch-plus
        if line.startswith("--packages-") or line in (
            ...
            raise click.ClickException(
                f"colcon flagfile '{path}': '{line}' conflicts with a launch-plus-managed "
...
def _cmd_verify(files: tuple[str, ...], lockfile_path: str) -> int:
    ...
    from launch_plus.indexer import parse_lockfile, parse_repos

    if not Path(lockfile_path).exists():
        click.echo(f"Error: Lockfile not found: {lockfile_path}")
        click.echo("Run 'launch-plus index' to generate it.")
        return 2
    ...
    if missing:
        ...
        click.echo("Run 'launch-plus index' to regenerate.")
        return 2
...
def update(
    ...
    from launch_plus.types import PackageLock
...
def build_pkg(
    ...
    from launch_plus.fetcher import FetchOptions, fetch_packages
