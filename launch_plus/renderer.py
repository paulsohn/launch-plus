...
# ---------------------------------------------------------------------------
# Action-based renderer (Phase 24e)
# ---------------------------------------------------------------------------
...
def render_resolved_xml_from_actions(
    package: str,
    launcher: str,
    actions: list,
    *,
    ...
    Uses ``action.serialize_resolved()`` instead of the old ``ResolvedNode``
    dispatch. Source-group nesting uses ``action._include_chain``.
    """
    if include_args is None:
        include_args = {}
    if initial_args is None:
